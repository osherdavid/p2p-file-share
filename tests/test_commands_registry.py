...
def test_register_command_populates_all_registries():
    @commands.register_command("TST")
    class SampleCommand(_BaseTestCommand):  # noqa: D401
...
def test_get_command_returns_same_instance_for_name_and_code():
...
def test_register_command_rejects_invalid_code_length():
...
def test_get_command_requires_selector():
    with pytest.raises(ValueError):
